...
const TOOLS: Tool[] = [
  {
    ...
    description:
      "Generate a production-ready Express.js TypeScript backend from an entity schema. " +
  ...
  },
  {
    name: "setup_database",
    description:
      "Connect to the user's database and create all tables derived from the entity schema. " +
  ...
  },
  {
    name: "open_in_ide",
    description:
      "Open a local project directory in the user's IDE (VS Code, Cursor, or WebStorm). " +
...
const server = new Server(
  { name: "ai-backend-generator", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
  ...
  try {
    switch (name) {
      case "generate_backend": {
        const result = await generateBackend(args as any);
        return {
      ...
      case "setup_database": {
        const result = await setupDatabase(args as any);
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
      }
      ...
      case "open_in_ide": {
        const result = await openInIde(args as any);
        return {
          content: [{ type: "text", text: JSON.stringify(result, null, 2) }],
        };
      }
