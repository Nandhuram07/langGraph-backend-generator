...
export type SupportedIde = "vscode" | "cursor" | "webstorm";

export interface OpenIdeInput {
  projectPath: string;
  ide?: SupportedIde;   // defaults to "vscode"
}
...
export async function openInIde(input: OpenIdeInput): Promise<OpenIdeResult> {
  ...
  exec(`${cmd} "${projectPath}"`, (err) => {
    if (err) console.error(`[open-ide] Failed to open: ${err.message}`);
  });
