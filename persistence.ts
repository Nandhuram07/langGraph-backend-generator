...
export class JSONFileSaver extends BaseCheckpointSaver {
  ...
  constructor(baseDir: string = ".persistence/sessions") {
    super();
    const isVercel = process.env.VERCEL === "1" || process.env.NODE_ENV === "production";
    const base = isVercel ? "/tmp" : process.cwd();
    this.baseDir = path.isAbsolute(baseDir) ? baseDir : path.resolve(base, baseDir);
    
    try {
      fs.ensureDirSync(this.baseDir);
    } catch (e) {
      console.warn(`[Persistence] Could not create directory ${this.baseDir}, session saving might fail.`, e);
    }
  }
