...
- Once Database, Features, and at least one entity are set, tell the user their ZIP is ready.`;
...
export async function getGraph() {
  ...
  if (process.env.REDIS_URL) {
    console.log("[Graph] Connecting to Redis via ioredis (Cloud Mode)...");
    try {
      const redis = new Redis(process.env.REDIS_URL, {
        tls: process.env.REDIS_URL.startsWith('rediss') ? {} : undefined,
        retryStrategy: (times) => {
          // Retry more aggressively at first, then slow down
          const delay = Math.min(times * 200, 3000);
          return delay;
        },
        // Setting to null ensures commands wait for connection rather than failing after N tries
        maxRetriesPerRequest: null, 
        connectTimeout: 20000, 
        // Optimization for cloud redis like Upstash
      ...
      redis.on('error', (err) => {
        // We log it, but the command won't fail; it will wait for reconnection
        console.error('[Graph] ioredis Connection Error:', err);
      });

      redis.on('connect', () => console.log('[Graph] Redis Connected. Status:', redis.status));
      redis.on('ready', () => console.log('[Graph] Redis Ready. Checkpointer active.'));

      checkpointer = new RedisSaver(redis as any);
      console.log("[Graph] Redis checkpointer initialized.");
    } catch (e) {
